Watchtower is itself packaged as a Docker container so installation is as simple as pulling the `todd2982/watchtower` image. If you are using ARM based architecture, pull the appropriate `todd2982/watchtower:armhf-<tag>` image from the [todd2982 Docker Hub](https://hub.docker.com/r/todd2982/watchtower/tags/).
...
docker run -d \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock \
  todd2982/watchtower container_to_watch --debug
```
